...
export const CourseCard = () => (
  <Card className="max-w-[350px] overflow-hidden group bg-white border shadow-sm">
    <div className="h-40 bg-zinc-100 flex items-center justify-center relative overflow-hidden uppercase">
      <BookOpen
        size={60}
        className="text-zinc-400 group-hover:scale-105 transition-transform duration-300"
      />
      ...
      {steps.map((step, i) => (
        <React.Fragment key={step.id}>
          <div className="flex flex-col items-center gap-2">
            ...
            <span
              className={`text-[10px] font-medium uppercase tracking-wider ${step.status === "current" ? "text-primary" : "text-muted-foreground"}`}
            >
              {step.name}
            </span>
          </div>
  ...
  const roadmap = [
    {
      ...
      title: "Beta Launch",
      status: "done",
      tasks: ["Core Components", "Standard Theme", "Documentation"],
    },
    {
      q: "Q2 2024",
    ...
    <div className="space-y-8 w-full max-w-2xl mx-auto">
      {roadmap.map((item, i) => (
        <div key={i} className="flex gap-4 relative">
          {i < roadmap.length - 1 && (
            <div className="absolute left-[17px] top-[40px] bottom-[-32px] w-[2px] bg-border" />
          )}
          <div
            className={`shrink-0 w-9 h-9 rounded-full border flex items-center justify-center z-10 ${
              item.status === "done"
                ? "bg-primary text-primary-foreground border-primary"
                : item.status === "active"
                  ? "border-primary text-primary"
    ...
    <Card className="w-full border shadow-sm bg-white overflow-hidden">
      ...
      <CardContent className="p-0">
        {modules.map((mod, i) => (
          <div
            key={i}
            className={`flex items-center justify-between p-3 border-b last:border-0 hover:bg-zinc-50 transition-colors cursor-pointer group ${
              mod.status === "current" ? "bg-primary/5" : ""
            }`}
          >
...
export const VerticalStepper = () => {
  const steps = [
    { title: "Personal Info", desc: "Contact details & address", status: "complete" },
    { title: "Verification", desc: "Upload ID & Documents", status: "current" },
    { title: "Payment Method", desc: "Link your bank account", status: "pending" },
  ];
    ...
    <div className="space-y-0 w-full max-w-sm mx-auto">
      {steps.map((step, i) => (
        <div key={i} className="flex gap-3 relative">
          {i < steps.length - 1 && (
            <div className="absolute left-[13px] top-[30px] bottom-0 w-[1px] bg-border" />
          )}
          <div className="flex flex-col items-center">
            <div
              className={`w-7 h-7 rounded-full border flex items-center justify-center font-medium z-10 transition-colors text-xs ${
                step.status === "complete"
                  ? "bg-primary text-primary-foreground border-primary"
                  : step.status === "current"
                    ? "border-primary text-primary"
